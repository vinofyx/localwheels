...
const FILTERS = ['MEMO DATE','MEMO NO','FROM BRANCH','TO BRANCH','VENDOR','VEHICLE NO'];

export default function ZeroFreightMemo() {
  const [filter, setFilter] = useState('MEMO DATE');
  const [from, setFrom] = useState('');
  const [to, setTo] = useState('');
  return (
    <div className="p-3 min-h-screen text-[13px]" style={{ backgroundColor: '#dce9f5' }}>
      <div className="text-center mb-2">
        <h2 className="font-bold text-[15px] tracking-wide underline">Zero Freight MEMO</h2>
      </div>
