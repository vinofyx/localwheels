...
const FILTERS = ['LR NO','LR DATE','BOOKING','DELIVERY','CONSIGNEE','COSIGNOR','MATERIAL NAME','BILLINGPARTY','BRANCH'];

export default function MemoPendingLR() {
  const [filter, setFilter] = useState('LR NO');
  const [value, setValue] = useState('');
  return (
    <div className="p-3 min-h-screen text-[13px]" style={{ backgroundColor: '#dce9f5' }}>
      <div className="text-center mb-2">
        <h2 className="font-bold text-[15px] tracking-wide underline">Memo Pending LR</h2>
      </div>
      <div className="bg-white rounded shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-3 py-2" style={{ backgroundColor: '#e8e8e8' }}>
          ...
          <input value={value} onChange={e => setValue(e.target.value)} className={`${inp} w-32`} />
