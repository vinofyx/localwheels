...
const FILTERS = ['ENTRY DATE','RECEIPT DATE','VEHICLE NO','BRANCH','VEH OWNER TYPE','VEHICLE VENDOR NAME'];

export default function FasTagStatement() {
  const [filter, setFilter] = useState('ENTRY DATE');
  ...
  return (
    <div className="p-3 min-h-screen text-[13px]" style={{ backgroundColor: '#dce9f5' }}>
      <div className="text-center mb-2">
        <h2 className="font-bold text-[15px] tracking-wide underline">FasTag Statement</h2>
      </div>
      <div className="bg-white rounded shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-3 py-2" style={{ backgroundColor: '#e8e8e8' }}>
          ...
          <input value={from} onChange={e => setFrom(e.target.value)} className={`${inp} w-32`} />
          <input value={to}   onChange={e => setTo(e.target.value)}   className={`${inp} w-32`} />
