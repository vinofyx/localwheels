...
export default function PendingPartBEWB() {
  const [filter,  setFilter]  = useState('EWB DATE');
  ...
  return (
    <div className="p-3 min-h-screen text-[13px]" style={{ backgroundColor: '#dce9f5' }}>
      ...
      {/* Filter bar */}
      <div className="bg-white rounded shadow-sm overflow-hidden mb-2">
        <div className="flex items-center gap-3 px-3 py-2" style={{ backgroundColor: '#e8e8e8' }}>
          <div className="flex items-center gap-2">
            ...
            <select value={filter} onChange={e => setFilter(e.target.value)}
              className={`${inp} w-36`}>
              <option>EWB DATE</option>
              <option>E-Way Bill NO</option>
            </select>
          </div>
          <input value={from} onChange={e => setFrom(e.target.value)}
            className={`${inp} w-28`}
            placeholder={filter === 'EWB DATE' ? 'From Date' : 'EWB No'} />
          <input value={to} onChange={e => setTo(e.target.value)}
            className={`${inp} w-28`}
            placeholder={filter === 'EWB DATE' ? 'To Date' : ''} />
