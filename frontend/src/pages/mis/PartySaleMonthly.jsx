...
export default function PartySaleMonthly() {
  const cur = new Date();
  ...
  const [partyType, setPartyType] = useState('Consignor');
  const [displayOn, setDisplayOn] = useState('Amount');
  const [activeTab, setActiveTab] = useState('Party Sale');
  const TABS = ['Party Sale', 'Party Collection'];

  return (
    <div className="min-h-screen bg-white text-[13px]">
      <div className="text-center py-2 border-b border-gray-300">
        <h2 className="font-bold text-[14px] underline">Party Sale (Monthly)</h2>
      </div>

      <div className="m-3 border border-gray-300 bg-[#f4f4f4] px-4 py-3">
        <div className="grid grid-cols-8 gap-3 mb-3 items-end">
          <div>
          ...
          <div>
            <div className="mb-0.5"><span className={lbl}>Party Type</span></div>
            <select value={partyType} onChange={e => setPartyType(e.target.value)} className={inp}>
