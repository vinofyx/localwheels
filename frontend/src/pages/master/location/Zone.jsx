...
const DIVISION_OPTIONS = ['ANDHRAPRADESH', 'TRANSPORT DIVISION'];

const INITIAL  = { zone: '', division: '' };
const COL_KEYS = ['zone', 'division', 'createdUser', 'createdDate', 'lastModify', 'modifyDate'];
...
export default function Zone() {
  ...
  const handleSave = () => {
    if (!form.zone.trim())               { toast.error('Zone is required');     return; }
    if (!form.division)                  { toast.error('Division is required'); return; }
    const now = new Date().toLocaleString('en-GB', {
      day: '2-digit', month: '2-digit', year: 'numeric',
      hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false,
    }).replace(',', '');
    setRows(r => [...r, { id: Date.now(), zone: form.zone.trim(), division: form.division, createdUser: 'admin', createdDate: now, lastModify: '', modifyDate: '' }]);
    setForm(INITIAL);
  ...
  const handleEdit = id => {
    ...
    if (row) { setForm({ zone: row.zone, division: row.division }); toast.success('Loaded for editing'); }
  };
      ...
      <div className="bg-white rounded shadow-sm px-5 py-3 mb-2">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            <label className="text-[13px] whitespace-nowrap w-12">Zone</label>
            <input
              name="zone"
              value={form.zone}
              onChange={set}
          ...
          <div className="flex items-center gap-3">
            <label className="text-[13px] whitespace-nowrap w-16">Division</label>
            <select
              name="division"
              value={form.division}
              onChange={set}
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr style={{ backgroundColor: '#0b8fd3', color: '#fff' }}>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">Zone {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">Division {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[16%]">Created User {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[16%]">Created Date {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[13%]">Last Modify {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[13%]">Modify Date {SortIcon}</th>
              ...
              {sliced.length === 0 ? (
                ...
                sliced.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-3 py-1.5 border-b border-gray-200 text-[#1565c0] font-medium">{row.zone}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200 text-[#1565c0] font-medium">{row.division}</td>
