...
const INITIAL  = { region: '', zone: '--Select--' };
const COL_KEYS = ['region', 'zone', 'createdUser', 'createdDate', 'lastModify', 'modifyDate'];
...
export default function Region() {
  ...
  const handleSave = () => {
    ...
    if (form.zone === '--Select--')    { toast.error('Zone is required');   return; }
    const now = new Date().toLocaleString('en-GB', {
      day: '2-digit', month: '2-digit', year: 'numeric',
      hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false,
    }).replace(',', '');
    setRows(r => [...r, { id: Date.now(), region: form.region.trim(), zone: form.zone, createdUser: 'admin', createdDate: now, lastModify: '', modifyDate: '' }]);
    setForm(INITIAL);
  ...
  const handleEdit = id => {
    ...
    if (row) { setForm({ region: row.region, zone: row.zone }); toast.success('Loaded for editing'); }
  };
  ...
  const zoneOptions = [...new Set(rows.map(r => r.zone).filter(z => z && z !== '--Select--'))];
    ...
    <div className="p-3 bg-[#cfd6de] min-h-screen text-[13px]">
      ...
      <div className="bg-white rounded shadow-sm px-5 py-3 mb-2">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            <label className="text-[13px] whitespace-nowrap">
              <span className="text-red-600">* </span>Zone
            </label>
            <select
              name="zone"
              value={form.zone}
            ...
            >
              ...
              <option>MIDDLE WEST ZONE</option>
              <option>NORTH ZONE STATE</option>
              <option>NORTH ZONE</option>
              <option>SOUTH ZONE</option>
              <option>WEST SOUTH ZONE</option>
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr style={{ backgroundColor: '#0b8fd3', color: '#fff' }}>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">Region {SortIcon}</th>
              ...
              {sliced.length === 0 ? (
                ...
                sliced.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-3 py-1.5 border-b border-gray-200 text-[#1565c0] font-medium">{row.region}</td>
