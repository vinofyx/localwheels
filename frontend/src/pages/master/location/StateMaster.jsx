...
const INITIAL  = { stateName: '', stateCode: '', gstin: '', withinStateLimit: '', outOfStateLimit: '' };
const COL_KEYS = ['stateName', 'stateCode', 'gstin', 'withinStateLimit', 'outOfStateLimit', 'createdUser', 'createdDate', 'lastModify', 'modifyDate'];
...
export default function StateMaster() {
  ...
  const handleSave = () => {
    ...
    setRows(r => [...r, {
      ...
      withinStateLimit: form.withinStateLimit.trim(),
      outOfStateLimit: form.outOfStateLimit.trim(),
  ...
  const handleEdit = id => {
    ...
    if (row) {
      setForm({ stateName: row.stateName, stateCode: row.stateCode, gstin: row.gstin, withinStateLimit: row.withinStateLimit, outOfStateLimit: row.outOfStateLimit });
  ...
  return (
    <div className="p-3 bg-[#cfd6de] min-h-screen text-[13px]">
      ...
      <div className="flex items-center mb-2">
        ...
        <h2 className="font-bold text-[15px] tracking-wide" style={{ fontVariant: 'small-caps' }}>State Master</h2>
      ...
      <div className="bg-white rounded shadow-sm px-6 py-4 mb-2">
        ...
        <div className="grid grid-cols-3 gap-x-8 gap-y-3 mb-3">
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap w-24"><span className="text-red-600">* </span>State Name</label>
            <input name="stateName" value={form.stateName} onChange={set}
              onKeyDown={e => e.key === 'Enter' && handleSave()}
              className="border border-blue-400 px-2 py-1 flex-1 text-[13px] focus:outline-none focus:border-blue-600" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap w-24">State Code</label>
            <input name="stateCode" value={form.stateCode} onChange={set}
        ...
        <div className="grid grid-cols-3 gap-x-8 gap-y-3">
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap w-24">GSTIN</label>
            <input name="gstin" value={form.gstin} onChange={set}
              className="border border-gray-400 px-2 py-1 flex-1 text-[13px] focus:outline-none focus:border-blue-500" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap w-36 text-[#e65100]">Within State Limit</label>
            <input name="withinStateLimit" value={form.withinStateLimit} onChange={set}
              className="border border-gray-400 px-2 py-1 flex-1 text-[13px] focus:outline-none focus:border-blue-500" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap w-36 text-[#e65100]">Out of State Limit</label>
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr style={{ backgroundColor: '#0b8fd3', color: '#fff' }}>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">State Name {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[8%]">State Code {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[12%]">GSTIN {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[10%]">Within State Limit {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[10%]">Out of State Limit {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[12%]">Created User {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[14%]">Created Date {SortIcon}</th>
            ...
            <tbody>
              {sliced.length === 0 ? (
                <tr>
                  <td colSpan={10} className="px-3 py-6 text-center text-gray-500 border border-gray-200">
                ...
                sliced.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.stateCode}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.gstin}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.withinStateLimit}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.outOfStateLimit}</td>
