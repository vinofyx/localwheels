...
const INITIAL  = { vehicleLoadType: '', capacity: '', deactive: false };
const COL_KEYS = ['vehicleLoadType', 'capacity', 'deactive', 'createdUser', 'createdDate', 'lastModify', 'modifyDate'];
...
export default function LoadType() {
  ...
  const handleSave = () => {
    if (!form.vehicleLoadType.trim()) { toast.error('Vehicle/Load Type is required'); return; }
    if (!form.capacity.trim())        { toast.error('Capacity(KG) is required');      return; }
    ...
    setRows(r => [...r, {
      id: Date.now(),
      vehicleLoadType: form.vehicleLoadType.trim(),
      capacity: form.capacity.trim(),
      deactive: form.deactive,
  ...
  const handleEdit = id => {
    ...
    if (row) {
      setForm({ vehicleLoadType: row.vehicleLoadType, capacity: row.capacity, deactive: row.deactive });
    ...
    <div className="p-3 bg-[#cfd6de] min-h-screen text-[13px]">
      ...
      <div className="flex items-center mb-2">
        ...
        <div className="flex-1 flex justify-center">
          <h2 className="font-bold text-[15px] tracking-wide" style={{ fontVariant: 'small-caps' }}>Load Type</h2>
        </div>
      ...
      <div className="bg-white rounded shadow-sm px-6 py-4 mb-2">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap"><span className="text-red-600">* </span>Vehicle/Load Type</label>
            <input name="vehicleLoadType" value={form.vehicleLoadType} onChange={set}
              onKeyDown={e => e.key === 'Enter' && handleSave()}
              className="border border-blue-400 px-2 py-1 w-52 text-[13px] focus:outline-none focus:border-blue-600" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap"><span className="text-red-600">* </span>Capacity(KG)</label>
            <input name="capacity" value={form.capacity} onChange={set}
              onKeyDown={e => e.key === 'Enter' && handleSave()}
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr style={{ backgroundColor: '#0b8fd3', color: '#fff' }}>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[20%]">Vehicle/Load Type {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[10%]">Capacity {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[8%]">DeActive {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[14%]">Created User {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">Created Date {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[12%]">Last Modify {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[12%]">Modify Date {SortIcon}</th>
            ...
            <tbody>
              {sliced.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-3 py-6 text-center text-gray-500 border border-gray-200">
                ...
                sliced.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-3 py-1.5 border-b border-gray-200 text-[#1565c0] font-medium">{row.vehicleLoadType}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.capacity}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200 text-center">
                      <input type="checkbox" checked={row.deactive} readOnly className="w-3.5 h-3.5" />
                    </td>
