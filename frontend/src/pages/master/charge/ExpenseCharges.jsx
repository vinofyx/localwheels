...
const SHOW_IN_OPTIONS = ['LR/MEMO EXPENSES', 'LR EXPENSES', 'MEMO EXPENSES', 'VEHICLE EXPENSES'];
const INITIAL  = { chargeHead: '', linkLedger: '', showIn: 'LR/MEMO EXPENSES' };
const COL_KEYS = ['chargeHead', 'linkLedger', 'showIn', 'createdUser', 'createdDate', 'lastModify', 'modifyDate'];

export default function ExpenseCharges() {
  ...
  const handleSave = () => {
    ...
    setRows(r => [...r, {
      ...
      linkLedger: form.linkLedger.trim(),
      showIn: form.showIn,
  ...
  const handleEdit = id => {
    ...
    if (row) { setForm({ chargeHead: row.chargeHead, linkLedger: row.linkLedger, showIn: row.showIn }); toast.success('Loaded for editing'); }
  };
    ...
    <div className="p-3 bg-[#cfd6de] min-h-screen text-[13px]">
      ...
      <div className="flex items-center mb-2">
        ...
        <div className="flex-1 flex justify-center">
          <h2 className="font-bold text-[15px] tracking-wide" style={{ fontVariant: 'small-caps' }}>Expense Charges</h2>
        </div>
      ...
      <div className="bg-white rounded shadow-sm px-6 py-4 mb-2">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap"><span className="text-red-600">* </span>Charge Head</label>
            <input name="chargeHead" value={form.chargeHead} onChange={set}
              onKeyDown={e => e.key === 'Enter' && handleSave()}
              className="border border-blue-400 px-2 py-1 w-56 text-[13px] focus:outline-none focus:border-blue-600" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap"><span className="text-red-600">* </span>Link Ledger</label>
            <input name="linkLedger" value={form.linkLedger} onChange={set}
              className="border border-gray-300 bg-gray-100 px-2 py-1 w-56 text-[13px] focus:outline-none focus:border-blue-500" />
            <button type="button"
          ...
          <div className="flex items-center gap-2">
            <label className="text-[13px] whitespace-nowrap"><span className="text-red-600">* </span>Show In</label>
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr style={{ backgroundColor: '#0b8fd3', color: '#fff' }}>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">Charge Head {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[18%]">LinkLedger {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[16%]">Show In {SortIcon}</th>
                ...
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[10%]">Last Modify {SortIcon}</th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8] w-[10%]">Modify Date {SortIcon}</th>
              ...
              {sliced.length === 0 ? (
                ...
                sliced.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-3 py-1.5 border-b border-gray-200 text-[#1565c0] font-medium">{row.chargeHead}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.linkLedger}</td>
                    <td className="px-3 py-1.5 border-b border-gray-200">{row.showIn}</td>
