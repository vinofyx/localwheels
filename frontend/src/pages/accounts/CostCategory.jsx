...
export default function CostCategory() {
  ...
  const handleSave = () => {
    ...
    setRows(r => [...r, { id: Date.now(), categoryName: form.categoryName.trim(), createdUser: 'admin', createdDate: now, lastModify: '', modifyDate: '' }]);
    setForm(INITIAL);
  ...
  const handleEdit = id => {
    ...
    if (row) { setForm({ categoryName: row.categoryName }); toast.success('Loaded for editing'); }
  };

  const handleRefresh = () => { setForm(INITIAL); setSearch(''); setPage(1); };

  const filtered = rows.filter(r => r.categoryName.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="p-3 bg-[#cfd6de] min-h-screen text-[13px]">
      ...
      <div className="bg-white rounded shadow-sm px-5 py-3 mb-2">
        <div className="flex items-center gap-3">
          <label className="text-[13px] whitespace-nowrap w-28">Category Name</label>
          <input
            value={form.categoryName}
            onChange={e => setForm({ categoryName: e.target.value })}
            onKeyDown={e => e.key === 'Enter' && handleSave()}
            className="border border-gray-400 px-2 py-1 w-64 text-[13px] focus:outline-none focus:border-blue-500"
          />
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2 text-[13px]">
            ...
            <input
              ...
              className="border border-gray-300 px-2 py-0.5 text-[13px] w-40 focus:outline-none focus:border-blue-400"
            />
          </div>
        </div>
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr style={{ backgroundColor: '#0b8fd3', color: '#fff' }}>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8]">
                  Category Name {SortIcon}
                </th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8]">
                  Created User {SortIcon}
                </th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8]">
                  Created Date {SortIcon}
                </th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8]">
                  Last Modify {SortIcon}
                </th>
                <th className="px-3 py-2 text-left font-medium border border-[#0a7ab8]">
                  Modify Date {SortIcon}
                </th>
                <th className="px-3 py-2 text-center font-medium border border-[#0a7ab8] w-16">
                  Edit {SortIcon}
                </th>
              </tr>
            </thead>
              ...
              {sliced.length === 0 ? (
                ...
                sliced.map((row, i) => (
                  <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-3 py-1.5 border border-gray-200">{row.categoryName}</td>
                    <td className="px-3 py-1.5 border border-gray-200 text-[#1565c0]">{row.createdUser}</td>
                    <td className="px-3 py-1.5 border border-gray-200">{row.createdDate}</td>
                    <td className="px-3 py-1.5 border border-gray-200 text-[#1565c0]">{row.lastModify}</td>
                    <td className="px-3 py-1.5 border border-gray-200">{row.modifyDate}</td>
